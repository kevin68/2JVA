    ...
    public void newGeneration()
    {
        ...
        for(int i = 0; i < this.nbRows; i++)
        {
            for(int j = 0; j < this.nbColumns; j++)
            {
                ...
                /*
                 * if(i > 0) { if(this.world[i-1][j].isAlive()) { nbNeighbours++; } if(j > 0) {
                 * if(this.world[i-1][j-1].isAlive()) { nbNeighbours++; } } if(j <
                 * this.nbColumns-1) { if(this.world[i-1][j+1].isAlive()) { nbNeighbours++; } }
                 * } if(i < this.nbRows-1) { if(this.world[i+1][j].isAlive()) { nbNeighbours++;
                 * } if(j > 0) { if(this.world[i+1][j-1].isAlive()) { nbNeighbours++; } } if(j <
                 * this.nbColumns-1) { if(this.world[i+1][j+1].isAlive()) { nbNeighbours++; } }
                 * } if(j > 0) { if(this.world[i][j-1].isAlive()) { nbNeighbours++; } } if(j <
                 * this.nbColumns-1) { if(this.world[i][j+1].isAlive()) { nbNeighbours++; } }
                 */
